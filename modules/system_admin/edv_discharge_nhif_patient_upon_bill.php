...
if ($_REQUEST['change'] == true) {
	$sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['discharge'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'discharge_nhif_upon_bill\' LIMIT 1;';
	$db->Execute($sql);
    $message = 'Updated';
}

$glob_obj->getConfig('discharge_nhif_upon_bill');
if (empty($GLOBAL_CONFIG['discharge_nhif_upon_bill'])) {
	$sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) '
		. 'VALUES (\'discharge_nhif_upon_bill\', \'0\', NULL, \'\', \'\', \'\', NOW(), \'\', NOW());';
	$db->Execute($sql);
}
?>
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td bgcolor="#99ccff" >
                                    &nbsp;&nbsp;<font color="#330066">Discharge NHIF Patients Upon Bill</font>
                                </td>
                        ...
                        <form  method="post" name="form">
                            <table border=0 cellspacing=1 cellpadding=5>
                                <tr>
                                    ...
                                    <td class="adm_item" align="right"><FONT  color="#0000cc"><b><?php echo 'Discharge NHIF Upon Bill?'; ?></b></FONT></td>
                                    <td  align="left">

                                        <select name="discharge">
                                            <?php
$discharge_nhif_upon_bill = $glob_obj->getConfigValue('discharge_nhif_upon_bill');
if ($discharge_nhif_upon_bill == 1) {
